...
export function loadTournament(username, localPlayerMode) {
  ...
  const ball = {
    ...
    speedY: canvas.width / 150,
  };
  const score = { left: 0, right: 0 };
  const players = {
    left: (canvas.height - paddle.height) / 2,
    right: (canvas.height - paddle.height) / 2,
  };
  const keys = {};
  ...
  async function update() {
    ...
    if (
      ball.x - ball.size < paddleLeftEdgeX + paddle.width &&
    ...
    if (
      ball.x + ball.size > paddleRightEdgeX &&
    ...
    if (localPlayerMode == true) {
      ...
      if (keys["ArrowDown"] && players.right < canvas.height - paddle.height)
        players.right += paddle.speed;
  ...
  async function handleLocalWinner() {
    ...
    if (g_count == 0 || g_count == 1) {
      winners.push(
        getWinner(score, pairings[g_count][0], pairings[g_count][1])
      );
      winners.reverse();
    }
    // if (score)
    ...
    if (g_count == 0) {
      ...
      await delay(4000).then(() => {
        ...
        if (winner === user_name) {
          ...
          showGameWinner(
            `TBA - Sorry you lost your chance. Please try again later`
          );
          // hideModal("modalGame");
          showModal("modalGame");
          window.history.pushState({}, "", "/play");
  ...
  async function checkForWinner() {
    if (score.left >= 3 || score.right >= 3) {
      isGameOver = true;
      ctx.fillStyle = "red";
      if (localPlayerMode) await handleLocalWinner();
      else await handleOnlineWinner();
    }
  }
  ...
  let player_count = 0;
  // let url = `wss://10.13.5.14:9090/ws/socket-server/`
  let url = `wss://10.13.5.14:9090/ws/socket-server/`;
  ...
  function initiateSocket() {
    ...
    gameSocket.onmessage = function (e) {
      ...
      if (data.game === "tic") return;
      if (data.player1 != user_name && data.player2 != user_name) return;
      if (data.type === "start" && data["status"] == "start") {
        ...
        if (data["player2"] == user_name) player2 = data["player1"];
        else player2 = data["player2"];
      ...
      if (data.type == "update") {
        ...
        if (leftPlayer) {
          if (data["key"] == "w" && players.right > 0)
            players.right -= paddle.speed;
          if (
            data["key"] == "s" &&
            players.right < canvas.height - paddle.height
          )
            players.right += paddle.speed;
        } else {
          if (data["key"] == "w" && players.left > 0)
            players.left -= paddle.speed;
          if (
            data["key"] == "s" &&
            players.left < canvas.height - paddle.height
          )
            players.left += paddle.speed;
        }
      } else if (
        data.type === "terminate" &&
        (data.player1 == user_name || data.player2 == user_name)
      ) {
        if (gameSocket !== "") {
          gameSocket.close();
        }
  ...
  function setupLocalTournament() {
    ...
    document
      .getElementById("formPlayerNames")
      ?.addEventListener("submit", function (event) {
        ...
        const playerNames = Array.from(
          document
            .getElementById("formPlayerNames")
            .querySelectorAll('input[id^="player"]')
        ).map((input) => input.value);

        for (let i = 0; i < playerNames.length; ++i) {
          if (!checkName(playerNames[i])) return;
        }
        // Check if all player names are unique
        if (hasDuplicates(playerNames)) {
          loadToast(
            "Player names must be unique. Please enter distinct names for each player."
          );
          return;
        }
  ...
  async function joinTournament(tournamentName) {
    ...
    await fetch(
      `/api/join/?username=${localStorage.getItem(
        "username"
      )}&tournament_name=${tournamentName}`,
      {
        method: "POST",
      ...
      .then(async (data) => {
        ...
        if (data.message === "Tournament joined successfully") {
          ...
          loadToast(
            "idiot ur already in the damn tournament STOP CHANGING PAGES !!STAY HERE PRICK"
          );
  ...
  function displayTournamentLobby(data) {
    ...
    document
      .getElementById("createTournForm")
