...
export function loadTicTac(username, localPlayerMode) {
  ...
  function local_init() {
    ...
    container.classList =
      "d-flex flex-row h-100 w-100 mh-100 mw-100 border border-0 border-black justify-content-around align-items-center overflow-auto font--argent";
    container.innerHTML = `
    ...
    board.style.scale =
      (docWinScreen.getBoundingClientRect().width / 300) * 0.5;
  ...
  function set() {
    if (
      ...
      (!localPlayerMode && turn !== currentPlayerSymbol)
    ) {
      return;
    }
    ...
    if (win(this)) {
      ...
      if (!localPlayerMode) handleOnlineDraw(false);
      return;
    } else {
      turn = turn === "X" ? "O" : "X";
      ...
      for (var j = 0; j < N_SIZE; j++) {
        ...
        cell.addEventListener("click", function () {
          ...
          if (this.innerHTML === EMPTY) {
            ...
            if (turn !== currentPlayerSymbol) return;
            gameSocket.send(
              JSON.stringify({
                type: "update",
                game: "tic",
  ...
  function handleMove(data) {
    ...
    if (
      data["key"] >= 0 &&
      data["key"] < boxes.length &&
      boxes[data["key"]].innerHTML === EMPTY
    ) {
